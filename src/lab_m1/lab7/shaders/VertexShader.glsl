...
void main()
{
    ...
    if (diffuse_light > 0)
    {
        //specular_light = material_ks * primeste_lumina * pow(max(dot(V, R), 0), material_shininess);
    ...
    float lumina = ambient_light + atenuare * (diffuse_light + specular_light);

    // TODO(student): Send color light output to fragment shader
    color_vertex = vec3(1);
    color_vertex *= object_color * lumina;
