...
void Lab8::Init()
{
    ...
    // Light & material properties
    {
        ...
        materialKd = glm::vec3(1, 0, 0);
        materialKs = glm::vec3(0, 1, 0);
        materialKa = glm::vec3(0.8f, 0.7f, 0);
        materialKe = glm::vec3(0, 0, 1);
        intensitate = glm::vec3(0.75, 0.4, 1);
    }
...
{
    {
        glm::vec4 newVec = glm::rotate(glm::mat4(1.0f), angleOX, glm::vec3(1, 0, 0)) * glm::vec4(lightDirection, 1);
        lightDirection = glm::normalize(glm::vec3(newVec));
    }

    {
        glm::vec4 newVec = glm::rotate(glm::mat4(1.0f), angleOY, glm::vec3(0, 1, 0)) * glm::vec4(lightDirection, 1);
        lightDirection = glm::normalize(glm::vec3(newVec));
    }
...

    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, glm::vec3(0, 1, 0.4));
    ...
    // Render cone
    {
        ...
        cout << "angleOX " << totalOX << endl;
        modelMatrix = glm::rotate(modelMatrix, totalOX, glm::vec3(0, 0, 1));
        modelMatrix = glm::scale(modelMatrix, glm::vec3(0.25f));
        // TODO(student): Add or change the object colors
...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    glUniform3f(material_ke, materialKe.x, materialKe.y, materialKe.z);

    int material_ka = glGetUniformLocation(shader->program, "material_ka");
    glUniform3f(material_ka, materialKa.x, materialKa.y, materialKa.z);

    int intensitateLight = glGetUniformLocation(shader->program, "intensity");
    glUniform3f(intensitateLight, intensitate.x, intensitate.y, intensitate.z);

    int object_color = glGetUniformLocation(shader->program, "object_color");
    glUniform3f(object_color, color.r, color.g, color.b);

    // TODO(student): Set any other shader uniforms that you need
    GLint is_spot = glGetUniformLocation(shader->program, "isSpot");
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_UP)) {
            angleOX = deltaTime * 0.5f;
            totalOX += deltaTime * 0.5f;
            isMoving = true;
        }
...
void Lab8::OnKeyPress(int key, int mods)
{
    ...
    if (key == GLFW_KEY_F) isSpot = !isSpot;
}
